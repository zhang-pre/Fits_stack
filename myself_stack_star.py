...
with open('positions/daocheng.txt', 'r') as file:
...
for i, filename in enumerate(filelist):
    ...
    if i > 0:
        ...
        dx, dy = int(round(ori_x-x)),int(round(ori_y-y))
    else:
        dx,dy = 0,0
...
output_filename = "stack_S9_daocheng_1~40.fit"
tempfilename = "original/phoebe-0001C.fit"
newHeader = fits.open(tempfilename)[0].header

fits.writeto(output_filename, data_stack, header=newHeader, overwrite=True)
